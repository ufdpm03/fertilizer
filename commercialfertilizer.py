...
if uploaded_file:
    ...
    # Add refined product search buttons
    st.markdown("### Search for This Fertilizer Online:")
    search_term = selected_fert.split("(")[0].strip() + " fertilizer"
    search_term = search_term.replace(" ", "+")
    urls = {
        "Lowe's": f"https://www.lowes.com/search?searchTerm={search_term}",
    ...
    for store, url in urls.items():
        st.markdown(f"<a href='{url}' target='_blank'><button style='margin:5px;padding:10px;background-color:#4CAF50;color:white;border:none;border-radius:4px;'>{store}</button></a>", unsafe_allow_html=True)
